...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
        ...
        # Output layout: [Q (dim), K (kv_dim), V (kv_dim)]
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        qkv = self.c_qkv(x)
        q = qkv[..., :self.q_dim].reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = qkv[..., self.q_dim:self.q_dim + self.kv_dim].reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
...
def eval_val_sliding_ttt(
    ...
    ttt_params = []
    ttt_param_ids = set()
    # Unfreeze last ttt_freeze_blocks blocks
