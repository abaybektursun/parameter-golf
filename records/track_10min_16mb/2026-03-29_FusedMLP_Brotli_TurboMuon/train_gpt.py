...
class Hyperparameters:
    ...
    muon_backend_steps = int(os.environ.get("MUON_BACKEND_STEPS", 4))  # Turbo-Muon: 4 iters (AOL compensates)
...
# --- Turbo-Muon: AOL-preconditioned Newton-Schulz with Polar Express coefficients ---
# From PR #1089 (Amsel et al., arXiv:2505.16932)
# AOL preconditioning contracts singular value spread, allowing NS5→NS4 (skip 1st iter)
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 4, eps: float = 1e-7) -> Tensor:
    """Turbo-Muon: AOL-preconditioned batched Newton-Schulz. G: (B,M,N) or (M,N).
    Default steps=4 (equivalent quality to standard NS5 due to AOL preconditioning)."""
    was_2d = G.ndim == 2
    if was_2d:
        G = G.unsqueeze(0)
    X = G.bfloat16()
    ...
    # AOL preconditioning: Gershgorin-based diagonal scaling
    A = X @ X.mT
    s = 1.0 / (A.abs().sum(dim=-1).sqrt() + eps)  # (B, M)
    X = s.unsqueeze(-1) * X
    # Polar Express iterations (skip first iter thanks to AOL)
    for i in range(steps):
        a, b, c = _AOL_POLAR_COEFFS[min(i, len(_AOL_POLAR_COEFFS) - 1)]
        A = X @ X.mT
        B = b * A + c * (A @ A)
        X = a * X + B @ X
    # Post-NS row then column L2 normalization
    X = X / (X.float().norm(dim=-1, keepdim=True).to(X.dtype) + eps)
    X = X / (X.float().norm(dim=-2, keepdim=True).to(X.dtype) + eps)
    if transposed:
        X = X.mT
    if was_2d:
        X = X.squeeze(0)
...
def main() -> None:
    ...
    log0(f"turbo_muon:AOL+PolarExpress ns_steps:{args.muon_backend_steps}")
