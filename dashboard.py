...
SOURCES = {
    "1xA100": {
        "results": "s3://fuelos-autoresearch/latest/experiments/002_autoresearch/results.tsv",
        "log": "s3://fuelos-autoresearch/logs/codex.log",
    },
    "8xH100": {
        "results": "s3://fuelos-autoresearch/latest-8xh100/experiments/002_autoresearch/results.tsv",
        "log": "s3://fuelos-autoresearch/logs/codex_8xh100.log",
    },
}
AWS_PROFILE = "fuelos"
PORT = 9090
...
cache = {
    "sources": {name: {"results": [], "codex_log": ""} for name in SOURCES},
    "last_sync": "",
}
...
function renderSource(name, src) {
  ...
  charts[name] = new Chart(document.getElementById('chart-' + name), {
    type: 'line',
    data: { datasets: [{ label: 'Post-Quant BPB (' + name + ')', data: chartData, borderColor: color, backgroundColor: color + '1a', fill: true, tension: 0.3, pointRadius: 5, pointBackgroundColor: chartData.map(p => p.y === bestBpb ? '#3fb950' : color) }] },
  ...
  let html = '';
  results.forEach((r, i) => {
    const isBest = parseFloat(r.post_quant_bpb) === bestBpb && r.status === 'keep';
...
async function refresh() {
  ...
  if (!activeTab) {
    ...
    document.getElementById('sections').innerHTML = names.map(n =>
      '<div class="source-section" data-name="' + n + '">' +
      '<div class="cards" id="cards-' + n + '"></div>' +
      '<div class="chart-wrap"><canvas id="chart-' + n + '"></canvas></div>' +
      '<h2>Experiments</h2>' +
      '<table><thead><tr><th>#</th><th>Commit</th><th>Val BPB</th><th>Post-Quant BPB</th><th>Artifact</th><th>Status</th><th>Description</th></tr></thead><tbody id="tbody-' + n + '"></tbody></table>' +
      '<h2>Agent Log</h2>' +
