"""
Experiment B: STE QAT on top of full meta stack.
...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and getattr(self, '_ste_enabled', False) and w.ndim == 2:
            with torch.no_grad():
                w32 = self.weight.float()
                clip_abs = torch.quantile(w32.abs(), QUANT_CLIP_Q, dim=1).clamp_min(1e-8)
                scale = clip_abs / 31.0
                w_clipped = torch.clamp(w32, -clip_abs[:, None], clip_abs[:, None])
                w_q = (torch.round(w_clipped / scale[:, None]) * scale[:, None]).to(x.dtype)
            w = w + (w_q - w).detach()  # STE: forward uses quantized, backward through original
...
def main() -> None:
    ...
    if args.ste_enabled:
        for module in base_model.modules():
            if isinstance(module, CastedLinear):
                module._ste_enabled = True
        log0("STE fake-quantize enabled on all CastedLinear layers")
