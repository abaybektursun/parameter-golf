"""
Experiment D: 8192 vocab.

Based on train_gpt.py from A_full_meta_stack. Uses an 8192-token vocabulary with
int8 quantization for the larger embedding (instead of fp16 passthrough).
Fewer layers (8) to compensate for the larger embedding table.
"""
...
# HYPERPARAMETERS
# -----------------------------
# Experiment D: 8192 vocab + int6 body + int8 embed + MLP 3x + 8 layers + sliding window eval
...
class Hyperparameters:
    ...
    embed_quant_range = int(os.environ.get("EMBED_QUANT_RANGE", 127))  # int8 for large embeddings
...
FP16_PASSTHROUGH_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get("FP16_PASSTHROUGH_PATTERNS", "").split(",")
    if pattern
)
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], quant_range: int = 31):
    ...
    for name, tensor in state_dict.items():
        ...
        # Special int8 quantization for large embedding (when fp16 passthrough is too expensive).
        if "tok_emb.weight" in name and t.numel() > QUANT_KEEP_FLOAT_MAX_NUMEL:
            stats["num_float_tensors"] += 1
            q, s = quantize_float_tensor(t, quant_range=127)  # int8 for embedding
            if s.ndim > 0:
