...
class Hyperparameters:
    ...
    # MLP-only layers experiment: comma-separated layer indices with no attention
    _no_attn_str = os.environ.get("NO_ATTN_LAYERS", "")
    no_attn_layers = set(int(x) for x in _no_attn_str.split(",") if x.strip()) if _no_attn_str.strip() else set()
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor | None, k_w: Tensor | None, v_w: Tensor | None, out_w: Tensor | None, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        no_attn_layers: set[int] | None = None,
    ):
        super().__init__()
        ...
        # MLP-only layers support
        no_attn = no_attn_layers or set()
        self.attn_layers = sorted(i for i in range(num_layers) if i not in no_attn)
        num_attn = len(self.attn_layers)
        self.attn_idx = {layer: ai for ai, layer in enumerate(self.attn_layers)}
        ...
        self.qo_bank = nn.Parameter(torch.empty(2 * num_attn, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * num_attn, kv_dim, model_dim))
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    has_attention=(i not in no_attn),
                )
        ...
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                if self.blocks[i].has_attention:
    ...
    def _init_weights(self) -> None:
        ...
        # Init attn banks: only for attention layers
        for ai in range(na):
            nn.init.orthogonal_(self.qo_bank.data[ai], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[na + ai])                   # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[ai], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[na + ai], gain=1.0)   # V
            self.qo_bank.data[na + ai].mul_(proj_scale)
        # Init MLP banks: all layers
        for i in range(n):
    ...
    def _attn_weights(self, layer_idx: int):
        """Return (q_w, k_w, v_w, out_w) for attention layers, or (None,)*4 for MLP-only."""
        if layer_idx not in self.attn_idx:
            return None, None, None, None
        ai = self.attn_idx[layer_idx]
        na = self.num_attn
        return self.qo_bank[ai], self.kv_bank[ai], self.kv_bank[na + ai], self.qo_bank[na + ai]
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w, k_w, v_w, out_w = self._attn_weights(i)
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w, k_w, v_w, out_w = self._attn_weights(bi)
...
def main() -> None:
    ...
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.has_attention and b.attn.use_xsa]
    no_attn_list = sorted(args.no_attn_layers) if args.no_attn_layers else []
    log0(f"no_attn_layers:{no_attn_list} attn_layers:{base_model.attn_layers} mlp_mult:{args.mlp_mult}")
    ...
    if args.skip_gptq:
        log0("SKIP_GPTQ=1: skipping GPTQ quantization")
        if distributed:
