...
5 physical transformer blocks are reused across 30 virtual layers (6 passes).
Per-virtual-layer LoRA adapters (rank-3) add a small additive residual before each
block, differentiating each pass through the shared weights. U-Net style skip
connections bridge the encoder and decoder halves of the virtual depth.
"""
...
# HYPERPARAMETERS
# -----------------------------
# Depth Recurrence: 5 physical blocks * 6 passes = 30 virtual layers
# LoRA rank-3 differentiates each virtual layer, U-Net skip connections
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,layer_scales",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.skip_weights = nn.Parameter(torch.ones(virtual_depth // 2, model_dim, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        half = self.virtual_depth // 2

        for i in range(self.virtual_depth):
            block = self.blocks[i % len(self.blocks)]
            x = x + self.layer_scales[i].to(dtype=x.dtype)[None, None, None] * self.lora_q[i](x)
            x = block(x, x0)
            if i < half:
                skips.append(x)
            elif len(skips) > 0:
                dec_idx = i - half
                if dec_idx < self.skip_weights.size(0):
...
def main() -> None:
    ...
    # LoRA 2D params go to Muon; layer_scales is a scalar param.
    lora_matrix_params = [p for p in base_model.lora_q.parameters() if p.ndim == 2]
    matrix_params.extend(lora_matrix_params)
    scalar_params.append(base_model.layer_scales)
