...
class Hyperparameters:
    ...
    # SLOT (Sample-specific LM Optimization at Test-time)
...
class GPT(nn.Module):
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return final hidden states (bsz, seq_len, model_dim) before lm_head."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
    ...
    if use_slot:
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for _step in range(args.slot_steps):
                slot_opt.zero_grad()
                adapted_logits = base_model.compute_logits((H + delta).to(torch.bfloat16)).float()
                slot_loss = F.cross_entropy(adapted_logits[:, :-1].reshape(-1, adapted_logits.size(-1)),
                                            y_batch[:, :seq_len-1].reshape(-1), reduction="mean")
                slot_loss.backward()
                slot_opt.step()
            with torch.no_grad():
                logits = base_model.compute_logits((H + delta.detach()).to(torch.bfloat16)).float()
...
def main() -> None:
    ...
    use_mixed = bool(int(os.environ.get("MIXED_QUANT", "0")))
    if use_mixed:
        ...
        n_int6 = int(os.environ.get("N_INT6_LAYERS", "10"))  # promote top N to int6
