...
class Hyperparameters:
    ...
    loss_truncation_pct = float(os.environ.get("LOSS_TRUNCATION_PCT", 0.95))  # clip at this percentile
...
class GPT(nn.Module):
    def __init__(
        ...
        self.value_residual = value_residual
        self.loss_truncation_pct = loss_truncation_pct
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.loss_truncation_pct < 1.0:
            threshold = torch.quantile(per_token_loss.detach(), self.loss_truncation_pct)
            mask = (per_token_loss <= threshold).float()
            per_token_loss = per_token_loss * mask
            main_loss = per_token_loss.sum() / mask.sum().clamp(min=1)
        else:
            main_loss = per_token_loss.mean()
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        loss_truncation_pct=args.loss_truncation_pct,
    ).to(device).bfloat16()
