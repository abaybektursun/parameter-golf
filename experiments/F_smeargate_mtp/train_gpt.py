...
Based on A_full_meta_stack/train_gpt.py with two additions:
  1. SmearGate: blends each token embedding with its predecessor via a learned gate.
  2. MTP auxiliary heads: extra linear heads that predict tokens k+1 ahead,
     providing an auxiliary loss during training (stripped before export).
"""
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            bsz_seq = x_pre_norm.size(0) * x_pre_norm.size(1) if x_pre_norm.ndim == 3 else x_pre_norm.size(0)
            seqlen = input_ids.size(1)
            mtp_loss_sum = logits.new_zeros(())
            mtp_count = 0
            for k, mtp_head in enumerate(self.mtp_heads):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                mtp_hidden = x_pre_norm[:, :valid_t, :].reshape(-1, x_pre_norm.size(-1)) if x_pre_norm.ndim == 3 else x_pre_norm[:valid_t]
...
def strip_mtp_weights(state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
    return {k: v for k, v in state_dict.items() if "mtp_heads" not in k}
...
def main() -> None:
    ...
    mtp_matrix_params = [p for name, p in base_model.mtp_heads.named_parameters() if p.ndim == 2]
    matrix_params.extend(mtp_matrix_params)
    ...
    if master_process:
        torch.save(strip_mtp_weights(base_model.state_dict()), "final_model.pt")
    ...
    quant_obj, quant_stats = quantize_state_dict_int8(strip_mtp_weights(base_model.state_dict()), quant_range=args.quant_range)
